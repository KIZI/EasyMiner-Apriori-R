...
class PMMLResult(arules: Seq[ARule]) {
  ...
  def toPMML = {
    val exprs = arules.flatMap(x => Seq(x.antecedent.getOrElse(emptyAntecedent), x.consequent)).map(collectExpression).reduceOption(_ ++ _)
    Template.apply(
